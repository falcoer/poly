...
class SerializedRunRenderer:
    ...
    def start(self, document: ReportDocument, command: str) -> None:
        ...
        progress = ""
        if capabilities.native_progress and self.actions and self.verbosity >= 0:
            self._progress_active = True
            progress = _native_progress_sequence(1, 0)
        self._inline_progress_active = bool(
            capabilities.cursor_updates and self.actions and self.verbosity >= 0
        )
        inline_progress = self._inline_progress() if self._inline_progress_active else ""
        self._write(
            progress
            + render_cli_start(
                document,
    ...
    def handle(self, event: RunEvent) -> None:
        ...
        if not capabilities.cursor_updates:
            ...
            self._write(rendered + "\n" + self._native_progress_update(event))
            return
        with self._lock:
            self._record_progress(event)
    ...
    def abort(self) -> None:
        if not self._closed and self._capabilities().interactive:
            clear_progress = _native_progress_sequence(0, 0) if self._progress_active else ""
            clear_inline = "\x1b[1A\x1b[J" if self._inline_progress_active else ""
            self._write(clear_inline + clear_progress + "\x1b[0m\n")
            self._progress_active = False
            self._inline_progress_active = False
        self._closed = True
