...
def _bare_clone(source: Path, target: Path) -> None:
    ...
    # Bare clones commonly pack the only branch and leave refs/ empty. GitHub
    # artifact ZIPs drop empty directories, which makes the extracted directory
    # fail Git's repository-shape check. Persist the real main ref as a valid
    # loose ref so the fixture survives artifact upload/download unchanged.
    main_commit = _git(source, "rev-parse", "refs/heads/main")
    loose_main = target / "refs" / "heads" / "main"
    loose_main.parent.mkdir(parents=True, exist_ok=True)
    loose_main.write_text(main_commit + "\n", encoding="utf-8")
